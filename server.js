...
const METADATA_FILE = path.join(__dirname, "servers.json");
...
app.use(session({
  secret: "kay_host_cyber_secret_9988",
  resave: false,
...
// Safety Helper to Prevent Undefined Errors
...
function stopServerProcess(serverId, callback) {
  const instance = ensureInstanceExists(serverId);
  addLog(serverId, "[System] Force Stopping Process & Clearing Port...");
  ...
  // Force Release Port (fuser + pkill combo)
  if (instance.port) {
    exec(`fuser -k -9 ${instance.port}/tcp || pkill -f "port ${instance.port}"`, () => {
      addLog(serverId, `[System] Port ${instance.port} forcefully freed.`);
      instance.status = "STOPPED";
      broadcastToApp(serverId, { type: "status", status: "STOPPED" });
...
server.listen(PORT, () => console.log(`KAY HOST Multi-Server live on port ${PORT}`));
